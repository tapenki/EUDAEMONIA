...
var bullet = preload("res://spheres/2/depths/bubble_bullet.tscn")
const wait_time = 0.2

func apply(ability_relay, applicant_data):
	if applicant_data.has("popgun"):
		ability_relay.damage_dealt_modifiers.connect(damage_dealt_modifiers)
	if applicants.has(ability_relay.source) and applicants[ability_relay.source].has("popgun"):
		applicant_data["popgun"] = applicants[ability_relay.source]["popgun"]
		ability_relay.damage_dealt_modifiers.connect(damage_dealt_modifiers)
...
func fire(direction, ability_relay):
	...
	{"subscription" = 2, "weapon" = true, "popgun" = true},
	direction * 450)
