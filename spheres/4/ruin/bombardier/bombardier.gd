...
var bullet = preload("res://spheres/4/ruin/bombardier/bombardier_bomb.tscn")
const wait_time = 0.5

func apply(ability_relay, applicant_data):
	if applicant_data.has("bombardier"):
		ability_relay.damage_dealt_modifiers.connect(damage_dealt_modifiers)
	if applicants.has(ability_relay.source) and applicants[ability_relay.source].has("bombardier"):
		applicant_data["bombardier"] = applicants[ability_relay.source]["bombardier"]
		ability_relay.damage_dealt_modifiers.connect(damage_dealt_modifiers)
...
func fire(direction, ability_relay):
	...
	{"subscription" = 2, "weapon" = true, "bombardier" = true},
	direction * 450)
