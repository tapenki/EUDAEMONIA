...
var bullet = preload("res://spheres/3/doubt/mold_bullet.tscn")
const wait_time = 0.35

func apply(ability_relay, applicant_data):
	if applicant_data.has("mold_machine"):
		ability_relay.damage_dealt_modifiers.connect(damage_dealt_modifiers)
	if applicants.has(ability_relay.source) and applicants[ability_relay.source].has("mold_machine"):
		applicant_data["mold_machine"] = applicants[ability_relay.source]["mold_machine"]
		ability_relay.damage_dealt_modifiers.connect(damage_dealt_modifiers)
...
func fire(direction, ability_relay):
	...
	{"subscription" = 2, "weapon" = true, "mold_machine" = true},
	direction * 450)
