...
var bullet = preload("res://spheres/6/night/moon_bullet/moon_bullet.tscn")
const wait_time = 0.5

func apply(ability_relay, applicant_data):
	if applicant_data.has("moon_dart"):
		ability_relay.damage_dealt_modifiers.connect(damage_dealt_modifiers)
		ability_relay.hits_left_modifiers.connect(hits_left_modifiers)
	if applicants.has(ability_relay.source) and applicants[ability_relay.source].has("moon_dart"):
		applicant_data["moon_dart"] = applicants[ability_relay.source]["moon_dart"]
		ability_relay.damage_dealt_modifiers.connect(damage_dealt_modifiers)
...
func fire(direction, ability_relay):
	...
	{"subscription" = 2, "weapon" = true, "moon_dart" = true},
	direction * 900)
...
func hits_left_modifiers(modifiers) -> void:
	modifiers["base"] += 1
