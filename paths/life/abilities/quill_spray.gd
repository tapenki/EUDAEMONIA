...
func damage_taken(_damage) -> void:
	var attack_scale = ability_handler.get_attack_scale({"base" : 0, "multiplier" : 1.5 * pressure_multiplier})
	var reach = 80 * attack_scale
	for entity in ability_handler.area_targets(global_position, reach):
		ability_handler.deal_damage(entity, {"base" : 0, "multiplier" : 2 * level * pressure_multiplier, "direction" : global_position.direction_to(entity.global_position)})
