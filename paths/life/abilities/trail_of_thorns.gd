		...
		if pain_walk:
			...
			if ability_handler.is_entity:
				...
				if pain_walk_counter == 0:
					ability_handler.deal_damage(ability_handler.owner, {"base" : 3, "multiplier" : 1.0, "skip_input_modifiers": true, "skip_output_modifiers": true, "skip_immunity": true}, Config.get_team_color(1, "tertiary"))
		else:
