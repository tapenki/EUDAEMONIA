...
func _unhandled_input(event: InputEvent) -> void:
	if Input.is_action_just_pressed("dark_harvest") and event.is_action("dark_harvest"):
		var target
		var to_target = INF
		for entity in ability_relay.area_targets(get_global_mouse_position(), 9999, pow(2, ability_relay.owner.group-1)):
			if entity.summoned:
		...
		if target:
			...
			2, ## inheritance
			Vector2()) ## velocity
			explosion_instance.ability_relay.inherited_damage["base"] += target.ability_relay.get_health().max_health * 0.2
			explosion_instance.ability_relay.inherited_damage["multiplier"] *= level
