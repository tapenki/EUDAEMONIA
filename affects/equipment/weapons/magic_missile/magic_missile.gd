...
func apply(ability_relay, applicant_data):
	if applicant_data.has("magic_missile"):
		ability_relay.damage_dealt_modifiers.connect(damage_dealt_modifiers)
	if applicants.has(ability_relay.source) and applicants[ability_relay.source].has("magic_missile"):
		applicant_data["magic_missile"] = applicants[ability_relay.source]["magic_missile"]
		ability_relay.damage_dealt_modifiers.connect(damage_dealt_modifiers)
...
func fire(direction, ability_relay):
	...
	{"subscription" = 2, "weapon" = true, "magic_missile" = true},
	direction * 600)
