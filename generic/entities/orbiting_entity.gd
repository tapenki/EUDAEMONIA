...
func movement(delta):
	...
	if loose:
		distance += speed * delta * ability_relay.speed_scale / 2
	else:
		distance = min(max_distance, distance + speed * delta * ability_relay.speed_scale / 2)
	
	orbitation += speed * delta * ability_relay.speed_scale / distance
