...
#func _ready() -> void:
	...
	#sprite_instance.modulate = Config.get_team_color(ability_relay.owner.group, "secondary")
...
#func damage_taken(_damage) -> void:
	#if charges > 0:
		#get_node("/root/Main").spawn_particles(particle_scene, global_position, 2, Config.get_team_color(ability_relay.owner.group, "secondary"))
		#charges -= 1
		#if charges == 0:
